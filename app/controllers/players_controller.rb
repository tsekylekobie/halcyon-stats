...
class PlayersController < ApplicationController
	...
	def show
		...
		if !player.success? # If player not found
			...
			@recent_matches = getRecentMatches(@player.id, "ranked")
			@stats = collectPlayerStats(@player.ign, @recent_matches)
		end
	end

	def getRecentMatches(id, gameMode)
		tStart = Time.new(2017, 01).utc.iso8601
		req = @db.matches({"filter[playerIds]" => id, "filter[gameMode]" => gameMode, "filter[createdAt-start]" => tStart, "sort" => "-createdAt"})
